    ...
    return (
        <>
            ...
            <div className={classes.itemsWrapper} onScroll={handleScroll}>
                ...
                {items.map(i => <ListItem className={classes.listItem} imageClassName={classes.itemImage} key={i._id} name={i.name} cat={i.cat} shouldBeColored={i.imageLink === "" && isAdmin} imageLink={i.imageLink} goToItemPage={goToItemPage} isArchived={i.archived} />)}
            </div>
        </>
    )
};
